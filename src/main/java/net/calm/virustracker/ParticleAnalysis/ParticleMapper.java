...
import net.calm.iaclasslibrary.Profile.PeakFinder;
import net.calm.iaclasslibrary.Segmentation.RegionGrower;
import net.calm.iaclasslibrary.Stacks.StackChecker;
...
import static net.calm.iaclasslibrary.IJUtilities.IJUtils.hideAllImages;
import static net.calm.iaclasslibrary.IJUtilities.IJUtils.resetRoiManager;
import static net.calm.iaclasslibrary.IO.DataWriter.*;
...
//            cleanUp();
    ...
    protected ParticleArray findParticles() {
        ...
        return findParticles(0, fociStack.getSize() - 1, UserVariables.getCurveFitTol(), fociStack, colocStack);
    }
    ...
    void buildHistograms(double[][] distances, int nBins, double max, double min, File resultsDir, boolean hidePlot) {
        ...
        try {
            saveValues((new Array2DRowRealMatrix(new double[][]{rt.getColumnAsDoubles(0), rt.getColumnAsDoubles(1)})).transpose().getData(),
...
//        c1Imp.close();
    ...
    public void saveDetectionsDataFile(Cell[] cells, String resultsDir) throws IOException {
        ...
        saveValues(detectionCoords, new File(String.format("%s%s%s", resultsDir, File.separator, PARTICLE_COORDS)), colHeadings.toArray(new String[]{}), null, false);
    }
